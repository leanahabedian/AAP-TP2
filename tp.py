...
for filename in filenames_cats:
  ...
  # choose one of the atributes extractor methods for cats
  av = ArmarVector(filename)
  #data[filename] = av.armar_vector_gris()
  #data[filename] = av.armar_vector_color()
  #data[filename]  = av.armar_vector_claro_oscuro_2x2()
...
for filename in filenames_dogs:
  ...
  # choose the same atributes extractor method for dogs
  av = ArmarVector(filename)
  #data[filename] = av.armar_vector_gris()
  #data[filename] = av.armar_vector_color()
  #data[filename]  = av.armar_vector_claro_oscuro_2x2()
  #data[filename]  = av.armar_vector_claro_oscuro_5x5()
  #data[filename]  = av.armar_vector_claro_oscuro_5x5_sin_overlapping()
  #data[filename]  = av.armar_vector_claro_5x5_sin_overlapping()
  data[filename]  = av.armar_vector_mahotas()
  #data[filename]  = av.armar_vector_mix()
  y.append(1)
...
## execute not mixed classifiers with differents n_estimator parameters
generate_many_random_forest(X_train, X_test, y_train, y_test, "[Random Forest LBP 10 estimators]", 10)
generate_many_random_forest(X_train, X_test, y_train, y_test, "[Random Forest LBP 100 estimators]", 100)
generate_many_random_forest(X_train, X_test, y_train, y_test, "[Random Forest LBP All estimators]", 1000)

## execute mixed classifiers (choose armar_vector_mix to run these lines)
...
#clf2 = generate_many_random_forest(train_last, test_last, y_train, y_test, "[Random Forest LBP]", 1000)
